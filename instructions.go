...
func opSignExtend(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	if back.Cmp(big.NewInt(31)) < 0 {
		...
		num := stack.pop()
		mask := back.Lsh(math.Big1, bit)
		mask.Sub(mask, math.Big1)
...
func opBlockhash(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	n := interpreter.intPool.get().Sub(interpreter.evm.BlockNumber, math.Big257)
...
func makeLog(size int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		...
		interpreter.evm.StateDB.AddLog(&statetype.Log{
			Address: contract.Address(),
			Topics:  topics,
			Data:    d,
			// This is a non-consensus field, but assigned here because
			// core/state doesn't know the current block number.
			BlockHeight: interpreter.evm.BlockNumber.Uint64(),
		})
...
func makePush(size uint64, pushByteSize int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		...
		integer := interpreter.intPool.get()
		stack.push(integer.SetBytes(hexutil.RightPadBytes(contract.Code[startMin:endMin], pushByteSize)))
